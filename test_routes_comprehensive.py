...
class TestFileUploadRoutes(BaseTestCase):
    ...
    def test_server_events_page_shows_invocations(self):
        ...
        page = response.get_data(as_text=True)
        self.assertIn(f'href="/{invocation_cid}"', page)
        self.assertIn(f'href="/{request_cid}"', page)
        self.assertIn(f'href="/{result_cid}"', page)
        self.assertIn(f'href="/{servers_cid}"', page)
