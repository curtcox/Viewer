...
def parse_import_payload(
    raw_payload: str,
) -> tuple[ParsedImportPayload | None, str | None]:
...
def load_import_payload(form: ImportForm) -> str | None:
    ...
    if source == "url":
        ...
        try:
            ...
            form.import_url.errors.append(
                "Failed to download JSON from the provided URL."
            )
            return None
    ...
    if source == "github_pr":
        ...
        github_token = (
            form.github_import_token.data.strip()
            if form.github_import_token.data
            else None
        )
...
def parse_source_entry(
    entry: Any, label_text: str, warnings: list[str]
) -> SourceEntry | None:
    ...
    if not isinstance(raw_path, str) or not expected_cid:
        warnings.append(
            f'{label_text} entry must include valid "path" and "cid" values.'
        )
        return None
    ...
    return SourceEntry(
        raw_path=raw_path, relative_path=candidate_path, expected_cid=expected_cid
    )
...
def source_entry_matches_export(
    entry: SourceEntry, local_bytes: bytes, warnings: list[str]
) -> bool:
...
def verify_import_source_category(
    ...
    for raw_entry in entries:
        ...
        absolute_path = resolve_source_entry(
            parsed_entry, base_path, base_resolved, warnings
        )
...
def verify_import_source_files(
    ...
    if raw_section is None:
        for _, label_text in selected_categories:
            warnings.append(
                f"No {label_text.lower()} were included in the import data."
            )
        return

    if not isinstance(raw_section, dict):
        warnings.append(
            "App source files section must be an object mapping categories to file entries."
        )
        return
