...
def test_validate_circular_imports_with_windows_paths():
    ...
    try:
        ...
        assert not result.passed, (
            "Should detect circular import with Windows-style paths"
        )
...
def test_validate_module_sizes_all_under_threshold():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        reporter = ValidationReporter()
        result = validate_module_sizes(
            [str(module1), str(module2)], reporter, threshold=100
        )
...
def test_validate_module_sizes_exceeds_threshold():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        reporter = ValidationReporter()
        result = validate_module_sizes(
            [str(small_module), str(large_module)], reporter, threshold=100
        )
...
def test_validate_module_sizes_reports_largest():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        reporter = ValidationReporter()
        result = validate_module_sizes(
            [str(module1), str(module2), str(module3)], reporter, threshold=100
        )
