...
def serve_directory(path):
    ...
    try:
        ...
        for entry in entries:
            ...
            name = escape(entry)
            # Normalize path and ensure it starts with /files/
            normalized_path = full_path.replace("\\", "/")
            if normalized_path.startswith("./"):
                normalized_path = normalized_path[2:]
            url = escape("/files/" + normalized_path)
        ...
        return {
            "output": "<html><body><h1>403 Forbidden</h1></body></html>",
            "status": 403,
        }
...
files_path = os.path.join(".", request_path)

if not os.path.exists(files_path):
    out = {
        "output": f"<html><body><h1>404 Not Found</h1>{files_path}</body></html>",
        "status": 404,
    }
elif os.path.isdir(files_path):
    out = serve_directory(files_path)
else:
    mime_type, _ = mimetypes.guess_type(files_path)
    ...
    try:
        ...
        out = {
            "output": "<html><body><h1>500 Internal Server Error</h1></body></html>",
            "status": 500,
        }
