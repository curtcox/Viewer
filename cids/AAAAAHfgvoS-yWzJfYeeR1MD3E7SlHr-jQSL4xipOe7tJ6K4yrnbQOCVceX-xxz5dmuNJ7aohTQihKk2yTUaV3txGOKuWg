...
from flask import Response, current_app, make_response, request as flask_request, stream_with_context
...
def _load_mcps(context: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    mcps_json = variables.get("mcps", "{}")

    try:
        mcps_data = json.loads(mcps_json) if isinstance(mcps_json, str) else mcps_json
        ...
        logger.warning(f"Failed to parse mcps configuration: {e}")
        return {}
...
def _get_cid_content(cid_path: str, context: Optional[Dict] = None) -> Optional[str]:
    ...
    if cid_path.startswith("reference_templates/"):
        try:
    ...
    try:
        ...
        cid_value = extract_cid_from_path(cid_path)
        if cid_value:
...
def _discover_server_tools(server_name: str, context: Optional[Dict] = None) -> List[Dict]:
    ...
    if ".py" in definition_cid or "def main(" in server_code:
        return _discover_python_tools(server_name, server_code)
    elif ".sh" in definition_cid or "@bash_command" in server_code:
