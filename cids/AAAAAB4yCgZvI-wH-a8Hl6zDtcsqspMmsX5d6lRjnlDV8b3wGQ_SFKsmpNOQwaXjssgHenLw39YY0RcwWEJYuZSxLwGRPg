...
def transform_response(response_details: dict, context: dict) -> dict:
    ...
    breadcrumb = _build_breadcrumb(request_path, gateway_config.get("gateway_name", "json_api"))
...
def _build_breadcrumb(request_path: str, gateway_name: str) -> str:
    ...
    parts = [f'<a href="/gateway/{escape(gateway_name)}">{escape(gateway_name)}</a>']
...
def _format_json_with_links(obj, link_config: dict, request_path: str = "", indent: int = 0) -> str:
    ...
    if isinstance(obj, str):
        ...
        if link_url:
            return f'<a href="{escape(link_url)}" class="json-link"><span class="json-string">"{escape(obj)}"</span></a>'
    ...
    if isinstance(obj, list):
        ...
        items = []
        for item in obj:
            formatted_item = _format_json_with_links(item, link_config, request_path, indent + 1)
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            formatted_value = _format_json_with_links(value, link_config, request_path, indent + 1)

            # Check for ID reference detection
            link_url = _detect_id_reference_link(key, value, link_config, request_path)
            if link_url:
                formatted_value = f"<a href='{escape(link_url)}' class='json-link'>{formatted_value}</a>"
