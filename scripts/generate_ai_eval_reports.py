...
def load_test_source(test_file_path: str) -> Optional[str]:
    ...
    try:
        ...
        print(
            f"Warning: Could not load test source {test_file_path}: {e}",
            file=sys.stderr,
        )
        return None
...
def highlight_json(data: Any) -> str:
    ...
    try:
        ...
        formatter = HtmlFormatter(
            style="friendly", cssclass="json-highlight", nowrap=True
        )
...
def format_external_call_html(call: Dict, call_idx: int) -> str:
    ...
    if resp_body:
        try:
            parsed_body = (
                json.loads(resp_body) if isinstance(resp_body, str) else resp_body
            )
            resp_body_html = f"""
            ...
            body_display = (
                resp_body
                if len(str(resp_body)) < 5000
                else str(resp_body)[:5000] + "\n... (truncated)"
            )
            resp_body_html = f"""
                <p><strong>Response Body:</strong></p>
    ...
    status_class = (
        "api-status-success" if str(status_code).startswith("2") else "api-status-error"
    )
...
def generate_detail_page(interaction_data: Dict, output_path: Path) -> None:
    ...
    interactions_section = (
        ...
        else '<p class="no-interactions">No interactions recorded</p>'
    )
...
def generate_index_page(interaction_files: List[Path], output_path: Path) -> None:
    ...
    for test in tests:
        ...
        doc_text = (
            test["doc"][:MAX_DOC_LENGTH] + "..."
            if len(test["doc"]) > MAX_DOC_LENGTH
            else test["doc"]
        )
    ...
    test_list = (
        "\n".join(test_items)
        if test_items
        else '<p class="no-tests">No tests found</p>'
    )
...
def main():
    ...
    if not interactions_dir.exists():
        print(
            f"Error: Interactions directory not found: {interactions_dir}",
            file=sys.stderr,
        )
        print(
            "Run the AI eval tests first to generate interaction data.", file=sys.stderr
        )
        return 1
    ...
    if not json_files:
        print(
            f"Warning: No interaction JSON files found in {interactions_dir}",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        "  View via source browser at: /source/test-results/ai-eval-reports/index.html"
    )
