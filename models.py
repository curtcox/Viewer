...
class Server(db.Model):
    ...
    definition_cid = db.Column(
        db.String(255), nullable=True, index=True
    )  # Track CID of definition
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
...
class EntityInteraction(db.Model):
    ...
    def __repr__(self) -> str:
        return (
            f"<EntityInteraction {self.entity_type}:{self.entity_name} {self.action}>"
        )
...
class ServerInvocationCIDs:
    ...
    def from_invocation(cls, invocation: "ServerInvocation") -> "ServerInvocationCIDs":
        return cls(
            result=ValidatedCID.try_from_string(
                getattr(invocation, "result_cid", None)
            ),
            servers=ValidatedCID.try_from_string(
                getattr(invocation, "servers_cid", None)
            ),
            variables=ValidatedCID.try_from_string(
                getattr(invocation, "variables_cid", None)
            ),
            secrets=ValidatedCID.try_from_string(
                getattr(invocation, "secrets_cid", None)
            ),
            request_details=ValidatedCID.try_from_string(
                getattr(invocation, "request_details_cid", None)
            ),
            invocation=ValidatedCID.try_from_string(
...
class ServerInvocation(db.Model):
    ...
    )  # Name of the server that was invoked
    result_cid = db.Column(
        db.String(255), nullable=False, index=True
    )  # CID of the result produced
    servers_cid = db.Column(
        db.String(255), nullable=True
    )  # CID of current servers definitions
    variables_cid = db.Column(
        db.String(255), nullable=True
    )  # CID of current variables definitions
    secrets_cid = db.Column(
        db.String(255), nullable=True
    )  # CID of current secrets definitions
    request_details_cid = db.Column(
        db.String(255), nullable=True
    )  # CID of request details JSON
    invocation_cid = db.Column(
        db.String(255), nullable=True, index=True
    )  # CID of this ServerInvocation JSON
    external_calls_cid = db.Column(
        db.String(255), nullable=True
    )  # CID of captured external calls JSON
