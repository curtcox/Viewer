...
class TestEnabledFieldImportExport(unittest.TestCase):
    ...
    def test_export_disabled_entities_preserves_enabled_false(self):
        ...
        disabled_alias = next((a for a in alias_entries if a['name'] == 'alias-False'), None)
        disabled_server = next((s for s in server_entries if s['name'] == 'server-False'), None)
        disabled_variable = next((v for v in variable_entries if v['name'] == 'variable-False'), None)
        disabled_secret = next((s for s in secrets_section['items'] if s['name'] == 'secret-False'), None)
        ...
        self.assertFalse(disabled_variable['enabled'], "Variable should have enabled=False")
    ...
    def test_export_enabled_entities_preserves_enabled_true(self):
        ...
        enabled_server = next((s for s in server_entries if s['name'] == 'server-True'), None)
        enabled_variable = next((v for v in variable_entries if v['name'] == 'variable-True'), None)
        enabled_secret = next((s for s in secrets_section['items'] if s['name'] == 'secret-True'), None)
        ...
        self.assertTrue(enabled_variable['enabled'], "Variable should have enabled=True")
