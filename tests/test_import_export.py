...
class TestImportExportRoutes(unittest.TestCase):
    ...
    def _load_section(self, payload: dict[str, Any], key: str):
        ...
        self.assertIsInstance(
            section_cid, str, f"{key} section should reference a CID string"
        )
        ...
        if entry is not None:
            ...
            if isinstance(entry, dict):
                ...
                self.assertIsInstance(
                    value, str, f"{key} CID entry must include string content"
                )
    ...
    def _load_export_payload(self) -> tuple[CID, dict[str, Any]]:
        ...
        self.assertIsNotNone(
            export_payload, "Stored export payload did not decode correctly"
        )
        ...
        with self.logged_in():
            with ExitStack() as stack:
                ...
                response = self.client.post(
                    "/export/size", data={"include_aliases": "y"}
                )
    ...
    def test_export_preview_lists_selected_items(self):
        ...
        self.assertRegex(
            html, r'name="selected_aliases"[^>]*value="alias-one"[^>]*checked'
        )
        self.assertRegex(
            html, r'name="selected_servers"[^>]*value="server-one"[^>]*checked'
        )
        self.assertRegex(
            html, r'name="selected_variables"[^>]*value="var-one"[^>]*checked'
        )
        self.assertIn(
            '<div class="text-muted small fst-italic">Secrets are not selected for export.</div>',
            html,
        )
    ...
    def test_export_preview_respects_disabled_and_template_filters(self):
        ...
        html = response.get_data(as_text=True)
        self.assertRegex(
            html, r'name="selected_aliases"[^>]*value="alias-disabled"[^>]*checked'
        )
        self.assertRegex(
            html, r'name="selected_aliases"[^>]*value="alias-regular"[^>]*checked'
        )
        self.assertRegex(
            html, r'name="selected_secrets"[^>]*value="secret-two"[^>]*checked'
        )
        self.assertNotIn(
            '<div class="text-muted small fst-italic">Secrets are not selected for export.</div>',
            html,
    ...
    def test_export_includes_selected_collections(self):
        ...
        secret_entry = self._find_entry_by_name(
            secrets_section.get("items") if secrets_section else None, "secret-one"
        )
    ...
    def test_export_and_import_preserve_enablement(self):
        ...
        disabled_alias = next(
            (a for a in alias_entries if a["name"] == "alias-disabled"), None
        )
        disabled_server = next(
            (s for s in server_entries if s["name"] == "server-disabled"), None
        )
        disabled_variable = next(
            (v for v in variable_entries if v["name"] == "variable-disabled"), None
        )
        disabled_secret = next(
            (s for s in secrets_section["items"] if s["name"] == "secret-disabled"),
            None,
        )
    ...
    def test_export_omits_disabled_items_without_selection(self):
        ...
        variable_entries = self._load_section(payload, "variables")
        self.assertIsNone(
            self._find_entry_by_name(variable_entries, "disabled-variable")
        )
    ...
    def test_export_requires_template_selection_for_templates(self):
        ...
        variable_entries = self._load_section(payload, "variables")
        self.assertIsNotNone(
            self._find_entry_by_name(variable_entries, "template-variable")
        )

        secrets_section = self._load_section(payload, "secrets")
        assert secrets_section is not None
        self.assertIsNotNone(
            self._find_entry_by_name(secrets_section.get("items"), "template-secret")
        )
        ...
        self.assertIsNotNone(
            self._find_entry_by_name(server_entries, "template-server")
        )
        ...
        assert secrets_section is not None
        self.assertIsNotNone(
            self._find_entry_by_name(
                secrets_section.get("items", []), "template-secret"
            )
        )
    ...
    def test_export_includes_disabled_templates_with_template_selection(self):
        ...
        alias_entries = self._load_section(payload, "aliases")
        disabled_alias_entry = self._find_entry_by_name(
            alias_entries, "disabled-template-alias"
        )
        ...
        server_entries = self._load_section(payload, "servers")
        disabled_server_entry = self._find_entry_by_name(
            server_entries, "disabled-template-server"
        )
        ...
        variable_entries = self._load_section(payload, "variables")
        disabled_variable_entry = self._find_entry_by_name(
            variable_entries, "disabled-template-variable"
        )
        ...
        secrets_section = self._load_section(payload, "secrets")
        assert secrets_section is not None
        disabled_secret_entry = self._find_entry_by_name(
            secrets_section["items"], "disabled-template-secret"
        )
    ...
    def test_export_includes_runtime_section(self):
        ...
        self.assertEqual(
            python_info.get("implementation"), platform.python_implementation()
        )
    ...
    def test_export_excludes_unreferenced_cids_by_default(self):
        ...
        cid_values = payload.get("cid_values", {})
        expected_server_cid = format_cid(
            generate_cid(server_definition.encode("utf-8"))
        )
    ...
    def test_import_reports_missing_selected_content(self):
        ...
        payload = json.dumps(
            {"aliases": [{"name": "alias-a", "definition": definition_text}]}
        )
    ...
    def test_import_reports_missing_cid_content(self):
        ...
        server_definition = (
            'print("hello, world! this is a long definition that exceeds 64 bytes")'
        )
        ...
        self.assertEqual(response.status_code, 200)
        expected_error = f'Server "server-c" definition with CID "{server_cid}" was not included in the import.'
    ...
    def test_import_verifies_app_source_matches(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assert_flash_present(
            "All python source files match the export.", response.data
        )
    ...
    def test_import_reports_mismatched_app_source(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assert_flash_present(
            'Source file "app.py" differs from the export.', response.data
        )
    ...
    def test_import_rejects_mismatched_cid_map_entry(self):
        ...
        mismatched_cid = format_cid(
            generate_cid(
                b"other mismatched content that is definitely longer than 64 bytes for sure"
            )
        )
        payload = json.dumps(
            {
        ...
        self.assertEqual(response.status_code, 200)
        mismatch_error = (
            f'CID "{mismatched_cid}" content did not match its hash and was skipped.'
        )
        self.assert_flash_present(mismatch_error, response.data)
        missing_error = f'Server "server-c" definition with CID "{mismatched_cid}" was not included in the import.'
        ...
        payload = json.dumps(
            {
                ...
                "secrets": {
                    "encryption": SECRET_ENCRYPTION_SCHEME,
                    "items": [{"name": "secret-b", "ciphertext": encrypted_secret}],
                },
                "cid_values": {
    ...
    def test_source_entry_helpers_validate_local_files(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                import_export._source_entry_matches_export(parsed, content, warnings)
            )
    ...
    def test_store_cid_entry_optional_behaviour(self):
        ...
        import_export._store_cid_entry(
            "test-cid", b"data", cid_entries, include_optional=False
        )
        self.assertEqual(cid_entries, {})

        import_export._store_cid_entry(
            "test-cid", b"data", cid_entries, include_optional=True
        )
        self.assertIn("test-cid", cid_entries)
        original_entry = cid_entries["test-cid"]

        import_export._store_cid_entry(
            "test-cid", b"updated", cid_entries, include_optional=True
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.app.app_context():
                with ExitStack() as stack:
                    stack.enter_context(
                        patch(
                            "routes.import_export.filesystem_collection.app_root_path",
                            return_value=base_path,
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_sections.get_aliases",
                            return_value=[
                                SimpleNamespace(
                                    name="example", definition=alias_definition
                                )
                            ],
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_sections.get_servers",
                            return_value=[],
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_sections.get_variables",
                            return_value=[],
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_sections.get_secrets",
                            return_value=[],
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_engine.get_uploads",
                            return_value=[],
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.change_history.gather_change_history",
                            return_value={},
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.cid_utils.store_cid_from_bytes",
                            side_effect=store_bytes_stub,
                        )
                    )
                    stack.enter_context(
                        patch(
                            "routes.import_export.export_engine.cid_path",
                            return_value="/downloads/export.json",
                        )
                    )
    ...
    def test_process_import_submission_returns_form_on_empty_payload(self):
        with self.app.test_request_context():
            ...
            with ExitStack() as stack:
                ...
                response = import_export._process_import_submission(
                    form, "note", render_form, parsed
                )
    ...
    def test_export_cid_values_as_utf8_strings(self):
        ...
        for cid, value in cid_values.items():
            ...
            self.assertNotIsInstance(
                value, dict, f"CID value for {cid} should not be a dict"
            )
    ...
    def test_snapshot_toggle_checked_hides_options_in_ui(self):
        ...
        snapshot_section = html[
            max(0, snapshot_section_start) : snapshot_section_start + 300
        ]
    ...
    def test_snapshot_unchecked_applies_default_settings(self):
        ...
        with self.app.app_context():
            ...
            server = Server(
                name="test-server", definition='print("test")', enabled=False
            )
    ...
    def test_snapshot_checked_uses_form_defaults(self):
        ...
        with self.app.app_context():
            ...
            disabled_alias = Alias(
                name="disabled-alias", definition="echo test", enabled=False
            )
    ...
    def test_import_cid_values_backward_compatibility(self):
        ...
        payload = json.dumps(
            {
                "servers": [
                    {"name": "old-format-server", "definition_cid": server_cid}
                ],
                "cid_values": {
                    server_cid: {"encoding": "utf-8", "value": server_definition},
                },
    ...
    def test_import_defaults_to_utf8_without_encoding(self):
        ...
        alias_definition = format_primary_alias_line(
            "literal", "test", content_with_unicode
        )
        alias_cid = format_cid(generate_cid(alias_definition.encode("utf-8")))
    ...
    def test_export_shows_only_100_most_recent(self):
        ...
        self.assertEqual(
            export_count, 100, f"Expected 100 exports, found {export_count}"
        )
    ...
    def test_import_generates_snapshot_export(self):
        ...
        alias_definition = format_primary_alias_line(
            "literal", None, "/servers/echo", alias_name="imported-alias"
        )
        payload = json.dumps(
            {
                "aliases": [
        ...
        with self.app.app_context():
            ...
            self.assertEqual(
                len(exports),
                initial_exports + 1,
                "Snapshot export should be created after import",
            )

            # Get the most recent export (the snapshot)
            snapshot_export = (
                Export.query.filter_by().order_by(Export.created_at.desc()).first()
            )
    ...
    def test_import_displays_snapshot_info_on_page(self):
        ...
        payload = json.dumps(
            {
                "aliases": [
                    {"name": "test-alias", "definition": "echo test", "enabled": True}
                ],
            }
        )
    ...
    def test_import_returns_snapshot_info_in_json_response(self):
        ...
        payload = {
            "aliases": [
                {"name": "api-alias", "definition": "echo api", "enabled": True}
            ],
        }
    ...
    def test_snapshot_export_contains_default_sections(self):
        ...
        with self.app.app_context():
            alias = Alias(name="test-alias", definition="echo test", enabled=True)
            server = Server(
                name="test-server", definition='print("test")', enabled=True
            )
        ...
        with self.app.app_context():
            ...
            self.assertIn("aliases", snapshot_payload)
            self.assertIsInstance(
                snapshot_payload["aliases"],
                str,
                "aliases section should be a CID string",
            )
            self.assertIn("servers", snapshot_payload)
            self.assertIsInstance(
                snapshot_payload["servers"],
                str,
                "servers section should be a CID string",
            )
            self.assertIn("variables", snapshot_payload)
            self.assertIsInstance(
                snapshot_payload["variables"],
                str,
                "variables section should be a CID string",
            )
    ...
    def test_snapshot_exports_from_same_state_have_identical_cids(self):
        ...
        with self.app.app_context():
            alias = Alias(
                name="consistent-alias", definition="echo consistent", enabled=True
            )
            server = Server(
                name="consistent-server", definition='print("ok")', enabled=True
            )
