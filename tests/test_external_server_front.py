import requests
from reference.templates.servers.definitions import front
...
def test_missing_api_token():
    assert front.main(FRONT_API_TOKEN="", dry_run=False)["output"]["error"] == "Missing FRONT_API_TOKEN"


def test_invalid_operation():
    assert front.main(FRONT_API_TOKEN="key", operation="unknown")["output"]["error"]["message"] == "Unsupported operation"


def test_get_conversation_requires_id():
    assert front.main(FRONT_API_TOKEN="key", operation="get_conversation")["output"]["error"]["message"] == "Missing required conversation_id"
...
def test_get_teammate_requires_id():
    assert front.main(FRONT_API_TOKEN="key", operation="get_teammate")["output"]["error"]["message"] == "Missing required teammate_id"
