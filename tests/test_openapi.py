...
class TestOpenAPI(unittest.TestCase):
    ...
    def test_openapi_schema_describes_interactions_endpoint(self):
        ...
        request_schema = post_definition["requestBody"]["content"]["application/json"][
            "schema"
        ]
        self.assertEqual(
            request_schema["$ref"], "#/components/schemas/InteractionRequest"
        )
    ...
    def test_openapi_schema_includes_entity_management_paths(self):
        ...
        for path, methods in expected_paths.items():
            ...
            for method in methods:
                self.assertIn(
                    method, paths[path], msg=f"Missing {method.upper()} for {path}"
                )
    ...
    def test_openapi_schema_describes_multi_format_responses(self):
        ...
        payload = response.get_json()
        interactions = payload["paths"]["/api/interactions"]["post"]["responses"][
            "200"
        ]["content"]
        for mimetype in [
            "text/html",
        ...
        alias_listing = payload["paths"]["/aliases"]["get"]["responses"]["200"][
            "content"
        ]
        ...
        server_detail = payload["paths"]["/servers/{server_name}"]["get"]["responses"][
            "200"
        ]["content"]
