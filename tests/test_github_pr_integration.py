...
class TestCreateExportPR(unittest.TestCase):
    ...
    def test_create_pr_branch_exists(
        self, mock_prepare, mock_get_repo, mock_get_client
    ):
...
class TestPrepareBootImageUpdate(unittest.TestCase):
    def test_prepare_boot_image_update_materializes_cids_and_does_not_write_generated_outputs(
        self,
    ):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            minimal_before = (ref_dir / "minimal.boot.source.json").read_text(
                encoding="utf-8"
            )
            ...
            aliases_section = [
                {
                    "name": "test-alias",
                    "definition_cid": alias_definition_cid,
                    "enabled": True,
                },
            ]
            ...
            self.assertEqual(
                (ref_dir / "minimal.boot.source.json").read_text(encoding="utf-8"),
                minimal_before,
            )
            self.assertEqual(
                (ref_dir / "boot.source.json").read_text(encoding="utf-8"), boot_before
            )
