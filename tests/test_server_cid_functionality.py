...
def test_server_cid_functionality():
    ...
    if isinstance(app, Mock):
        print(
            "Skipping test due to Flask-Login conflicts when running with unittest discover"
        )
        return
    ...
    with app.app_context():
        ...
        expected_cid = encode_cid_length(
            len(definition1.encode("utf-8"))
        ) + _base64url_encode(definition1.encode("utf-8"))
        ...
        expected_definition_cid = encode_cid_length(
            len(definition.encode("utf-8"))
        ) + _base64url_encode(definition.encode("utf-8"))
        ...
        assert updated_cid_record.file_data == "print('Updated server code')".encode(
            "utf-8"
        )
