import requests
from reference.templates.servers.definitions import gorgias
...
def test_invalid_operation():
    assert gorgias.main(domain="test", email="a@b.com", GORGIAS_API_KEY="key", operation="unknown")["output"]["error"]["message"] == "Unsupported operation"


def test_get_ticket_requires_id():
    assert gorgias.main(domain="test", email="a@b.com", GORGIAS_API_KEY="key", operation="get_ticket")["output"]["error"]["message"] == "Missing required ticket_id"


def test_update_ticket_requires_id():
    assert gorgias.main(domain="test", email="a@b.com", GORGIAS_API_KEY="key", operation="update_ticket")["output"]["error"]["message"] == "Missing required ticket_id"
