...
def test_gateway_server_proxies_to_target(
    ...
    response = client.get("/gateway/test?target_server=https://api.example.com", follow_redirects=True)
    assert response.status_code == 200
...
def test_gateway_server_converts_json_urls_to_links(
    ...
    mock_response.status_code = 200
    mock_response.headers = {"Content-Type": "application/json"}
    mock_response.content = b'{"user_url": "https://api.example.com/users/1", "repos_url": "/repos/test"}'
    mock_response.text = '{"user_url": "https://api.example.com/users/1", "repos_url": "/repos/test"}'
    ...
    response = client.get("/gateway?target_server=https://api.example.com", follow_redirects=True)
    assert response.status_code == 200
...
def test_gateway_server_handles_path_routing(
    ...
    response = client.get("/gateway/users/123?target_server=https://api.example.com", follow_redirects=True)
    
    # Even if the request fails, the gateway should have attempted to process it
    # (not return a 404 or routing error)
    assert response.status_code in [200, 500]  # Either success or connection error, not 404
