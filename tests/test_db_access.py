...
class TestDBAccess(unittest.TestCase):
    ...
    def test_find_cids_by_prefix_filters_and_orders_matches(self):
        ...
        self.assertEqual(
            [cid.path for cid in dotted_matches], ["/alpha.one", "/alpha.two"]
        )
    ...
    def test_page_view_helpers(self):
        ...
        filtered_page = paginate_page_views(
            page=1, per_page=5, start=cutoff, end=base_time
        )
    ...
    def test_server_invocation_helpers(self):
        ...
        self.assertEqual(
            [invocation.server_name for invocation in all_invocations],
            ["demo", "other"],
        )

        recent_only = get_server_invocations(start=now - timedelta(minutes=2))
        self.assertEqual(
            [invocation.server_name for invocation in recent_only], ["demo"]
        )
