...
def test_add_query_filters(memory_client, requires_openrouter_api_key, ai_interaction_tracker):
    ...
    has_sort = (
        'sort=' in updated_text or
        'order=' in updated_text or
        'orderBy=' in updated_text or
        'sortBy=' in updated_text
    )
    ...
    assert len(updated_text) < 500, f"Query string too long: {len(updated_text)} characters"
