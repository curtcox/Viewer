...
def test_add_retry_configuration(memory_client, requires_openrouter_api_key, ai_interaction_tracker):
    ...
    attempts_field = (
        retry.get('max_attempts') or
        retry.get('maxAttempts') or
        retry.get('attempts') or
