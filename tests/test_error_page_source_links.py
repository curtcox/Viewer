...
class TestErrorPageSourceLinks(unittest.TestCase):
            ...
            try:
                ...
                with patch("db_access.get_aliases") as mock_get_aliases:
                    ...
                    sqlite_error = sqlite3.OperationalError(
                        "no such column: alias.definition"
                    )
                    ...
                    self.assertIsNotNone(
                        aliases_func, "aliases route should be registered"
                    )
    ...
    def test_debug_mode_override_mechanism_works(self):
        ...
        self.assertTrue(
            hasattr(debug_app, "handle_exception"),
    ...
    def test_source_link_generation_for_various_file_types(self):
        ...
        with self.app.test_request_context("/test"):
            try:
                ...
                self.assertGreater(
                    len(source_links), 0, "Should have at least one source link"
                )

                # Check for specific project files that should be linked
                python_files_linked = any(
                    "/source/" in link and ".py" in link for link in source_links
                )
                self.assertTrue(
                    python_files_linked, "Should have source links for Python files"
                )
            ...
            try:
                ...
                if len(code_blocks) > 0:
                    ...
                    for block in code_blocks:
                        line_numbers_in_code.extend(
                            re.findall(line_number_pattern, block)
                        )

                    # Should have line numbers in the code context
                    self.assertGreater(
                        len(line_numbers_in_code),
                        0,
                        "Should have line numbers in code blocks",
                    )
                    ...
                    self.assertGreater(
                        len(error_markers),
                        0,
                        "Should have >>> error markers for the exact error line",
                    )
                else:
                    # If no code blocks, that's also valid - just check we have the basic structure
                    print(
                        "DEBUG: No code blocks found, checking for basic error structure"
                    )
                    self.assertIn(
                        "ValueError", html_content, "Should show the exception type"
                    )
    ...
    def test_exception_chain_display(self):
        ...
        with self.app.test_request_context("/test"):
            try:
                ...
                self.assertIn(
                    "RuntimeError", html_content, "Should show the outer exception"
                )
                self.assertIn(
                    "ValueError", html_content, "Should show the inner exception"
                )
                ...
                self.assertIn(
                    "Caused by:", html_content, "Should have 'Caused by:' labels"
                )
    ...
    def test_error_page_html_structure_and_styling(self):
        ...
        with self.app.test_request_context("/test"):
            try:
                ...
                self.assertIn(
                    "<!DOCTYPE html>", html_content, "Should have proper DOCTYPE"
                )
                ...
                self.assertIn(
                    "card", html_content, "Should use Bootstrap card component"
                )
                self.assertIn(
                    "btn btn-primary",
                    html_content,
                    "Should have Bootstrap button styling",
                )
                ...
                self.assertIn(
                    "Debugging Tips", html_content, "Should have debugging tips section"
                )
    ...
    def test_comprehensive_file_discovery_in_stack_trace(self):
        ...
        with self.app.test_request_context("/test"):
            ...
            try:
                ...
                self.assertGreater(
                    len(stack_trace), 0, "Should have stack trace frames"
                )
    ...
    def test_error_page_shows_comprehensive_project_file_links(self):
        ...
        with self.app.test_request_context("/test"):
            try:
                ...
                project_files = [
                    link for link in source_links if not link.startswith("venv/")
                ]
                self.assertGreater(
                    len(project_files),
                    0,
    ...
    def test_stack_trace_building_with_project_files(self):
        ...
        with self.app.app_context():
            ...
            try:
                ...
                frames_with_links = [
                    frame for frame in stack_trace if frame.get("source_link")
                ]

                # Should have at least one frame with a source link
                self.assertGreater(
                    len(frames_with_links),
                    0,
                    "Should have at least one frame with a source link",
                )
