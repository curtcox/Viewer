...
class TestAuthorizationIntegration:
    ...
    def test_rejected_api_request_returns_json_401(self, mock_authorize, memory_client):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=401, message="API authentication required"
        )

        response = memory_client.get(
            "/api/routes", headers={"Accept": "application/json"}
        )
        assert response.status_code == 401
    ...
    def test_rejected_api_request_returns_json_403(self, mock_authorize, memory_client):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=403, message="API access denied"
        )
    ...
    @patch("app.authorize_request")
    def test_rejected_text_request_returns_plain_text(
        self, mock_authorize, memory_client
    ):
        """Test that rejected text requests return plain text."""
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=401, message="Text authentication required"
        )
    ...
    def test_post_request_authorization(self, mock_authorize, memory_client):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=403, message="POST not allowed"
        )

        response = memory_client.post(
            "/aliases/new", headers={"Accept": "application/json"}
        )
        assert response.status_code == 403
    ...
    def test_put_request_authorization(self, mock_authorize, memory_client):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=403, message="PUT not allowed"
        )

        response = memory_client.put(
            "/api/test", headers={"Accept": "application/json"}
        )
        assert response.status_code == 403
    ...
    def test_delete_request_authorization(self, mock_authorize, memory_client):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=403, message="DELETE not allowed"
        )

        response = memory_client.delete(
            "/api/test", headers={"Accept": "application/json"}
        )
        assert response.status_code == 403
    ...
    @patch("app.authorize_request")
    def test_mixed_authorized_and_rejected_requests(
        self, mock_authorize, memory_client
    ):
        ...
        mock_authorize.return_value = AuthorizationResult(
            allowed=False, status_code=401, message="Now authentication required"
        )
        response = memory_client.get("/")
        assert response.status_code == 401
