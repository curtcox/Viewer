...
class TestEnhancedErrorPageIntegration(unittest.TestCase):
    ...
    def test_aliases_route_error_shows_comprehensive_source_links(self):
        ...
        with self.app.test_request_context("/test"):
            try:
                ...
                self.assertIn(
                    'href="/source/tests/test_enhanced_error_pages.py"', response
                )
    ...
    def test_comprehensive_file_discovery_includes_all_project_files(self):
        ...
        with self.app.app_context():
            ...
            self.assertTrue(
                any("routes/aliases.py" in path for path in comprehensive_paths)
            )
            self.assertTrue(
                any("routes/core.py" in path for path in comprehensive_paths)
            )
            ...
            self.assertTrue(
                any(
                    "tests/test_enhanced_error_pages.py" in path
    ...
    def test_error_page_template_renders_with_source_links(self):
        ...
        self.assertIn(
            '<i class="fas fa-external-link-alt me-1 text-primary"></i>', response
        )
    ...
    def test_source_browser_serves_comprehensive_files(self):
        ...
        with self.app.test_client() as client:
            ...
            )  # 200 for file, 302 for redirect to login
    ...
    def test_error_handling_robustness(self):
        ...
        with self.app.test_request_context("/test"):
            ...
            with patch(
                ...
                side_effect=Exception("Stack trace building failed"),
            ):
                try:
                    raise ValueError("Original error")
                except ValueError as exc:
